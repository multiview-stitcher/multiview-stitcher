...
def test_expand_dims_returns_real_zarr_array(zarr_format):
    ...
    assert zarr_utils.expand_dims(source, 0) is source
...
def test_stack_returns_real_zarr_array(zarr_format):
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        stacked_axis1 = zarr_utils.stack([a, b], axis=1)
...
@pytest.mark.parametrize("zarr_format", [2, 3])
def test_concatenate_chunk_aligned(zarr_format):
    with tempfile.TemporaryDirectory() as tmpdir:
        a = _write_zarr(
            tmpdir, "a.zarr", np.full((4, 4), 1, np.uint16), (2, 2), zarr_format
        )
        ...
        assert zarr_utils.is_chunk_aligned_concatenate([a, b], axis=0)
        concatenated = zarr_utils.concatenate([a, b], axis=0)
...
@pytest.mark.parametrize("zarr_format", [2, 3])
def test_concatenate_chunk_size_one_axis_always_aligned(zarr_format):
...
def test_concatenate_rejects_non_chunk_aligned():
    with tempfile.TemporaryDirectory() as tmpdir:
...
def test_virtual_transforms_compose(zarr_format):
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        ea = zarr_utils.expand_dims(a, 1)
        eb = zarr_utils.expand_dims(b, 1)

        stacked = zarr_utils.stack([ea, eb])
