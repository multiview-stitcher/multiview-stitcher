...
from multiview_stitcher.bayesian_fusion import (
    PSFType,
    bayesian_fusion,
    estimate_psf,
    make_gaussian_psf,
...
# ---------------------------------------------------------------------------
# bayesian_fusion: direct array interface
# ---------------------------------------------------------------------------
...
def test_bayesian_fusion_2d_psf_types(psf_type):
    views, weights = _make_views(3, (32, 32))
    result = bayesian_fusion(views, weights, psf_type=psf_type, n_iterations=3)
    assert result.shape == (32, 32)
    assert result.dtype == views.dtype
    assert np.all(np.isfinite(result))


def test_bayesian_fusion_3d():
    views, weights = _make_views(2, (8, 24, 24))
    result = bayesian_fusion(views, weights, n_iterations=2)
    assert result.shape == (8, 24, 24)
    assert np.all(np.isfinite(result))


def test_bayesian_fusion_explicit_psfs():
    views, weights = _make_views(2, (32, 32))
    psfs = [make_gaussian_psf(1.0, ndim=2), make_gaussian_psf(2.0, ndim=2)]
    result = bayesian_fusion(views, weights, psfs=psfs, n_iterations=3)
    assert result.shape == (32, 32)
    assert np.all(np.isfinite(result))


def test_bayesian_fusion_wrong_psf_count_raises():
    ...
    with pytest.raises(ValueError, match="n_views"):
        bayesian_fusion(views, weights, psfs=psfs, n_iterations=1)


def test_bayesian_fusion_tikhonov():
    views, weights = _make_views(2, (24, 24))
    result = bayesian_fusion(views, weights, n_iterations=3, lambda_reg=1e-3)
    assert result.shape == (24, 24)
    assert np.all(np.isfinite(result))


def test_bayesian_fusion_with_nan_views():
    ...
    w[1, :, 16:] = 1.0
    result = bayesian_fusion(views, w, n_iterations=3)
    assert np.all(np.isfinite(result))


def test_bayesian_fusion_psf_sigma_px():
    views, weights = _make_views(2, (24, 24))
    result = bayesian_fusion(views, weights, psf_sigma_px=2.0, n_iterations=2)
    assert result.shape == (24, 24)


def test_bayesian_fusion_output_spacing():
    views, weights = _make_views(2, (24, 24))
    result = bayesian_fusion(
        views, weights,
...
def test_fuse_pipeline_bayesian(ndim):
    """bayesian_fusion integrates correctly with fusion.fuse()."""
    kwargs = dict(
